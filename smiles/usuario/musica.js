...
const loadUserLikes = async (email) => {
  if (!email) return [];
  try {
    const snap = await getDocs(query(collection(db, 'musicaLikes'), where('email', '==', email.trim())));
...
const saveMusicLike = async (user, track) => {
  ...
  const likeData = {
    ...
    email: user.email,
    trackId: track.id,
    slug: (track.titulo || '').toLowerCase().replace(/á/g, 'a').replace(/é/g, 'e').replace(/í/g, 'i').replace(/ó/g, 'o').replace(/ú/g, 'u').replace(/ñ/g, 'n').replace(/[^a-z0-9]+/g, '-').trim('-'),
    titulo: track.titulo || '',
    artista: track.idioma || track.cantante || 'CumpleWii',
    subtitulo: track.tag || '',
    url: track.url,
    artworkUrl: track.poster || '',
    creado: serverTimestamp(),
    actualizado: serverTimestamp()
  };
  await setDoc(doc(db, 'musicaLikes', docId), likeData);
};

const deleteMusicLike = async (user, trackId) => {
  const docId = `${user.usuario.trim().toLowerCase()}_${trackId.trim().toLowerCase()}`;
  await deleteDoc(doc(db, 'musicaLikes', docId));
};
...
const toggleLoopMode = () => {
  ...
  if (loopMode === 'none') {
    ...
    Mensaje('Bucle desactivado ⏹️', 'success');
  }
...
const setupEvents = () => {
  ...
  if (plsList) {
    listen(plsList, 'click', async (e) => {
      ...
      const canEdit = user && (user.rol === 'admin' || user.rol === 'gestor' || (user.rol === 'editor' && (!c.email || c.email === user.email)));
      if (canEdit) {
        editandoId = c.id;
...
export const render = () => {
  ...
  const canUpload = user.rol === 'admin' || user.rol === 'gestor' || user.rol === 'editor';
