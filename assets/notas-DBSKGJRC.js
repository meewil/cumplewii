import{t as e}from"./wii-DFanSMW3.js";import{B as t,b as n,o as r,s as i}from"./widev-Czq8ue36.js";import{n as a}from"./vendor-Dkcx8wGc.js";import{n as o}from"./index-U_klNYw1.js";import{A as s,C as c,E as l,N as u,S as d,h as f,k as p,v as m,y as h}from"./firebase-BY5GISzq.js";import{n as g,t as _}from"./firebase-BOoMX3kr.js";var v=[],y=null,b=null,x=null,S=`wi_notas_cache`,C=`wi_notas_cache_time`,w=100,T=1200,E=300*1e3,D=()=>n(`wiSmile`)||{},O=e=>{try{localStorage.setItem(S,JSON.stringify(e)),localStorage.setItem(C,Date.now().toString())}catch{}},k=()=>{try{return JSON.parse(localStorage.getItem(S)||`[]`)}catch{return[]}},A=()=>{let e=parseInt(localStorage.getItem(C)||`0`);return Date.now()-e<E},j=()=>{localStorage.removeItem(S),localStorage.removeItem(C)},M=[{id:`Cielo`,hex:`#0EBEFF`,bg:`rgba(14,190,255,.12)`,tx:`var(--tx)`,rgb:`14,190,255`},{id:`Dulce`,hex:`#FF5C93`,bg:`rgba(255,92,147,.12)`,tx:`var(--tx)`,rgb:`255,92,147`},{id:`Paz`,hex:`#10B981`,bg:`rgba(16,185,129,.12)`,tx:`var(--tx)`,rgb:`16,185,129`},{id:`Mora`,hex:`#8B5CF6`,bg:`rgba(139,92,246,.12)`,tx:`var(--tx)`,rgb:`139,92,246`},{id:`Sol`,hex:`#F59E0B`,bg:`rgba(245,158,11,.12)`,tx:`var(--tx)`,rgb:`245,158,11`}],N=()=>{let n=D();if(!n.email)return location.replace(`/`),``;let r=n.nombre||n.usuario||n.email||_.currentUser?.email||``;return`
  ...
  </div>`},P=()=>{Q();let e=D();if(!e.email)return o.navigate(`/`);let t=e.email||_.currentUser?.email;a(document).on(`click.wn`,`#wnNueva`,()=>z(t)).on(`click.wn`,`#wnSync`,()=>I(t)).on(`click.wn`,`.wn_card`,function(e){a(e.target).closest(`.wn_toolbar, .wn_colors`).length||B(a(this).data(`id`))}).on(`input.wn`,`.wn_titulo, .wn_contenido`,function(){V(a(this).closest(`.wn_card`).data(`id`),t)}).on(`click.wn`,`.wn_pin`,function(e){e.stopPropagation(),U(a(this).closest(`.wn_card`).data(`id`),t)}).on(`click.wn`,`.wn_color`,function(e){e.stopPropagation(),a(this).closest(`.wn_card`).find(`.wn_colors`).toggleClass(`show`)}).on(`click.wn`,`.wn_color_opt`,function(e){e.stopPropagation();let n=a(this).closest(`.wn_card`);W(n.data(`id`),a(this).data(`color`),t),n.find(`.wn_colors`).removeClass(`show`)}).on(`click.wn`,`.wn_del`,function(e){e.stopPropagation(),y=a(this).closest(`.wn_card`).data(`id`),a(`#wnEliminar`).addClass(`show`)}).on(`click.wn`,`#wnCancel, #wnEliminar`,e=>{a(e.target).is(`#wnCancel, #wnEliminar`)&&(a(`#wnEliminar`).removeClass(`show`),y=null)}).on(`click.wn`,`#wnConfirm`,()=>G(t)).on(`click.wn`,e=>{a(e.target).closest(`.wn_colors, .wn_color`).length||a(`.wn_colors`).removeClass(`show`)}),F(t),x=()=>{!document.hidden&&!A()&&L(t,!0)},document.addEventListener(`visibilitychange`,x)},F=e=>{let t=k();t.length&&A()?(v=t,R(),a(`#wnGrid`).html(Y(v)),K(!0,`Cache`)):L(e,!1)},I=async e=>{a(`#wnSync`).addClass(`spinning`),j(),await L(e,!1),a(`#wnSync`).removeClass(`spinning`),r(`Sincronizado ✓`,`success`,1500)},L=async(e,t=!1)=>{try{K(!1,`Cargando...`),v=(await m(d(p(g,`wiNotas`),l(`email`,`==`,e),h(w)))).docs.map(e=>({id:e.id,...e.data()})),R(),O(v),a(`#wnGrid`).html(Y(v)),K(!0)}catch(e){if(console.error(`❌ Notas:`,e),K(!1,`Offline`),!t){let e=k();e.length?(v=e,a(`#wnGrid`).html(Y(v)),r(`Usando caché local 📦`,`warning`,2e3)):a(`#wnGrid`).html(X(`fa-wifi-slash`,`Sin conexión`,`Verifica tu internet`))}}},R=()=>{v.sort((e,t)=>e.pin===t.pin?(t.fecha?.seconds||0)-(e.fecha?.seconds||0):t.pin?1:-1)},z=async e=>{let t=`nota_${Date.now()}`,{usuario:n=``,nombre:i=``}=D(),o={id:t,titulo:``,contenido:``,color:`Cielo`,pin:!1,email:e,usuario:i||n||e,fecha:{seconds:Date.now()/1e3}};v.unshift(o),O(v),a(`#wnGrid`).html(Y(v)),setTimeout(()=>{a(`.wn_card[data-id="${t}"]`).addClass(`editing`).find(`.wn_titulo`).focus()},50);try{await c(s(g,`wiNotas`,t),{...o,fecha:u()}),K(!0),r(`Nueva nota ✨`,`success`,1200)}catch(e){console.error(`❌`,e),v=v.filter(e=>e.id!==t),O(v),a(`#wnGrid`).html(Y(v)),r(`Error al crear`,`error`)}},B=e=>{let t=a(`.wn_card[data-id="${e}"]`);a(`.wn_card.editing`).not(t).removeClass(`editing`),t.toggleClass(`editing`),t.hasClass(`editing`)&&t.find(`.wn_titulo`).focus()},V=(e,t)=>{clearTimeout(b),b=setTimeout(()=>H(e,t),T)},H=async(e,t)=>{let n=a(`.wn_card[data-id="${e}"]`),i=n.find(`.wn_titulo`).text().trim(),o=n.find(`.wn_contenido`).text().trim(),l=v.find(t=>t.id===e);if(l&&!(l.titulo===i&&l.contenido===o)){l.titulo=i,l.contenido=o,O(v),n.addClass(`saving`);try{await c(s(g,`wiNotas`,e),{id:e,titulo:i,contenido:o,color:l.color,pin:l.pin,email:t,usuario:l.usuario,fecha:u()}),K(!0),n.removeClass(`saving`).addClass(`saved`),setTimeout(()=>n.removeClass(`saved`),800)}catch(e){console.error(`❌`,e),n.removeClass(`saving`),r(`Error al guardar`,`error`)}}},U=async(e,t)=>{let n=v.find(t=>t.id===e);if(n){n.pin=!n.pin,R(),O(v),a(`#wnGrid`).html(Y(v));try{await c(s(g,`wiNotas`,e),{...n,fecha:u()}),K(!0),r(n.pin?`Fijada 📌`:`Desanclada`,`success`,1e3)}catch(e){console.error(`❌`,e),n.pin=!n.pin,R(),O(v),a(`#wnGrid`).html(Y(v))}}},W=async(e,t,n)=>{let r=v.find(t=>t.id===e);if(!r||r.color===t)return;let i=r.color;r.color=t,O(v),a(`#wnGrid`).html(Y(v));try{await c(s(g,`wiNotas`,e),{...r,fecha:u()}),K(!0)}catch(e){console.error(`❌`,e),r.color=i,O(v),a(`#wnGrid`).html(Y(v))}},G=async()=>{if(!y)return;let e=y;y=null,a(`#wnEliminar`).removeClass(`show`);let t=[...v];v=v.filter(t=>t.id!==e),O(v),a(`.wn_card[data-id="${e}"]`).addClass(`deleting`),setTimeout(()=>a(`#wnGrid`).html(Y(v)),250);try{await f(s(g,`wiNotas`,e)),r(`Eliminada 🗑️`,`success`,1e3)}catch(e){console.error(`❌`,e),v=t,O(v),a(`#wnGrid`).html(Y(v)),r(`Error al eliminar`,`error`)}},K=(e,t)=>{a(`.wn_dot`).removeClass(`active error`).addClass(e?`active`:`error`),a(`.wn_dotxt`).text(t||(e?`Online`:`Offline`))},q=e=>M.find(t=>t.id===e)||M[0],J=e=>{if(!e)return`Ahora`;let t=e.toDate?.()||new Date((e.seconds||0)*1e3),n=new Date;n.setHours(0,0,0,0);let r=new Date(n);return r.setDate(r.getDate()-1),t>=n?t.toLocaleTimeString(`es`,{hour:`2-digit`,minute:`2-digit`}):t>=r?`Ayer`:t.toLocaleDateString(`es`,{day:`numeric`,month:`short`})},Y=e=>e?.length?e.map(e=>{let n=q(e.color);return`
