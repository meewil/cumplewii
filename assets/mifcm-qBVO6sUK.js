import{b as e,o as t}from"./widev-Czq8ue36.js";import{n}from"./vendor-Dkcx8wGc.js";var r=()=>e(`wiSmile`),i=[{id:`wiihope_all`,txt:`Todos los usuarios`},{id:`wiihope_paz`,txt:`Mensajes de paz`},{id:`wiihope_esperanza`,txt:`Esperanza`},{id:`wiihope_blog`,txt:`Blog`},{id:`wiihope_biblia`,txt:`Biblia`},{id:`wiihope_felicitaciones`,txt:`Felicitaciones`}],a=[{id:`home`,txt:`Inicio`},{id:`messages`,txt:`Mensajes`},{id:`blog`,txt:`Blog`},{id:`bible`,txt:`Biblia`},{id:`quotes`,txt:`Citas`},{id:`music`,txt:`Musica`},{id:`settings`,txt:`Ajustes`},{id:`dashboard`,txt:`Dashboard`}],o=[{id:`esperanza`,txt:`Esperanza`},{id:`blog`,txt:`Blog`},{id:`biblia`,txt:`Biblia`},{id:`citas`,txt:`Citas`},{id:`sistema`,txt:`Sistema`},{id:`felicitacion`,txt:`Felicitacion`}],s=(e=``)=>String(e).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`).replace(/'/g,`&#039;`),c=e=>String(n(e).val()||``).trim(),l=()=>{let e=r();return!e||e.rol!==`admin`?`<div class="fcm_page"><div class="fcm_empty"><i class="fas fa-ban"></i> Acceso denegado.</div></div>`:`
  ...
  </div>`},u=()=>{let e=r();!e||e.rol!==`admin`||(n(document).off(`.fcm`),n(document).on(`click.fcm`,`#fcm_btn_build`,f).on(`change.fcm keyup.fcm`,`#fcm_title,#fcm_body,#fcm_topic,#fcm_screen,#fcm_type,#fcm_slug`,f).on(`click.fcm`,`#fcm_btn_demo`,p).on(`click.fcm`,`[data-copy]`,function(){m(n(this).data(`copy`))}),f())},d=()=>{n(document).off(`.fcm`)};function f(){let e=c(`#fcm_title`)||`WiiHope`,t=c(`#fcm_body`)||`Tienes un nuevo mensaje.`,r=c(`#fcm_topic`)||`wiihope_all`,i=c(`#fcm_screen`)||`messages`,a=c(`#fcm_type`)||`sistema`,o=c(`#fcm_slug`),l={title:e,body:t,screen:i,type:a};o&&(l.slug=o);let u=[`Topic: ${r}`,``,`Notification title: ${e}`,`Notification text: ${t}`,``,`Custom data:`,...Object.entries(l).map(([e,t])=>`${e}: ${t}`)].join(`
`),d={message:{topic:r,notification:{title:e,body:t},data:l,android:{priority:`HIGH`,notification:{channel_id:`wiihope_general`,click_action:`OPEN_WIIHOPE`}}}};n(`#fcm_console`).html(`
  ...
  `),n(`#fcm_json`).text(JSON.stringify(d,null,2))}function p(){n(`#fcm_title`).val(`Nueva reflexion en WiiHope`),n(`#fcm_body`).val(`Hay una lectura nueva para fortalecer tu fe hoy.`),n(`#fcm_topic`).val(`wiihope_blog`),n(`#fcm_screen`).val(`blog`),n(`#fcm_type`).val(`blog`),n(`#fcm_slug`).val(`biblia_amor_verdadero`),f(),t(`Ejemplo de blog cargado`,`success`)}async function m(e){let r=n(e),i=r.is(`input,textarea`)?r.val():r.text();try{await navigator.clipboard.writeText(i),t(`Copiado al portapapeles`,`success`)}catch{let e=n(`<textarea>`).val(i).appendTo(`body`).select();document.execCommand(`copy`),e.remove(),t(`Copiado`,`success`)}}export{d as cleanup,u as init,l as render};