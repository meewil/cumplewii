import{E as e,L as t,b as n,o as r}from"./widev-Czq8ue36.js";import{A as i,C as a,E as o,N as s,S as c,T as l,_ as u,b as d,h as f,k as p}from"./firebase-BY5GISzq.js";import{n as m}from"./firebase-BOoMX3kr.js";var h=null,g=n(`misProyectosLinkwii`)||[],_=g.length>0?g[0].slug:null,v=[{hex:`#FFFFFF`,name:`auto`},{hex:`#FFDA34`,name:`Oro`},{hex:`#3cd741`,name:`Success`},{hex:`#ffa726`,name:`Warning`},{hex:`#00a8e6`,name:`Info`},{hex:`#0EBEFF`,name:`Cielo`},{hex:`#FF5C69`,name:`Dulce`},{hex:`#29C72E`,name:`Paz`},{hex:`#7000FF`,name:`Mora`},{hex:`#21273B`,name:`Futuro`},{hex:`#dddddd`,name:`Offline`}],y=[``,`fas fa-link`,`fab fa-instagram`,`fab fa-tiktok`,`fab fa-youtube`,`fab fa-whatsapp`,`fab fa-facebook`,`fab fa-spotify`,`fab fa-x-twitter`,`fab fa-linkedin`,`fab fa-telegram`,`fas fa-globe`,`fas fa-store`,`fas fa-envelope`,`fas fa-phone`],b=e=>e?e.includes(`instagram.com`)?`fab fa-instagram`:e.includes(`tiktok.com`)?`fab fa-tiktok`:e.includes(`youtube.com`)||e.includes(`youtu.be`)?`fab fa-youtube`:e.includes(`whatsapp.com`)||e.includes(`wa.me`)?`fab fa-whatsapp`:e.includes(`facebook.com`)||e.includes(`fb.me`)?`fab fa-facebook`:e.includes(`spotify.com`)?`fab fa-spotify`:e.includes(`twitter.com`)||e.includes(`x.com`)?`fab fa-x-twitter`:e.includes(`linkedin.com`)?`fab fa-linkedin`:e.includes(`t.me`)||e.includes(`telegram`)?`fab fa-telegram`:e.includes(`pinterest.com`)?`fab fa-pinterest`:`fas fa-link`:``,x=async()=>n(`wiSmile`)?`
  ...
  `).join(``),c=n.links||[],u=c.map((e,t)=>`
    ...
    <div class="cr_section">
      ...
      ${c.length>0?`<div class="cr_links_grid">${u}</div>`:`<div class="ad_empty" style="padding:3vh 0; font-size:0.88rem;">Sin enlaces. Añade uno.</div>`}
    </div>
  `,document.getElementById(`btn_update_info`)?.addEventListener(`click`,async e=>{let a=e.currentTarget;t(a,!0,`Guardando`);try{await l(i(m,`linkwiis`,n.slug),{desc:document.getElementById(`edit_desc`).value,logo:document.getElementById(`edit_logo`).value,color:document.getElementById(`edit_color`).value,actualizado:s()}),r(`Apariencia guardada ✨`,`success`)}catch{r(`Error al guardar`)}t(a,!1,`<i class="fas fa-save"></i> Guardar`)}),document.getElementById(`btn_add_link_arr`)?.addEventListener(`click`,async()=>{let e=[...n.links||[],{titulo:``,url:``,icon:``}];await l(i(m,`linkwiis`,n.slug),{links:e})}),document.querySelectorAll(`.btn_rm_link`).forEach(e=>{e.addEventListener(`click`,async e=>{if(!confirm(`¿Estás seguro de eliminar este enlace? Esta acción no se puede deshacer.`))return;let t=parseInt(e.currentTarget.dataset.index),a=[...n.links||[]];a.splice(t,1);try{await l(i(m,`linkwiis`,n.slug),{links:a}),r(`Enlace eliminado ✨`,`success`)}catch{r(`Error al eliminar`)}})}),document.getElementById(`btn_save_links`)?.addEventListener(`click`,async e=>{let a=e.currentTarget;t(a,!0,`Guardando`);let o=n.links.filter(e=>e.titulo.trim()||e.url.trim());o.push({titulo:``,url:``,icon:``});try{await l(i(m,`linkwiis`,n.slug),{links:o,actualizado:s()}),r(`Enlaces guardados ✨`,`success`)}catch{r(`Error al guardar enlaces`)}t(a,!1,`<i class="fas fa-save"></i> Guardar`)}),document.getElementById(`btn_del_main`)?.addEventListener(`click`,async()=>{if(confirm(`¿Eliminar definitivamente el proyecto /${n.slug}? Esta acción no se puede deshacer.`))try{await f(i(m,`linkwiis`,n.slug)),_=null,r(`Proyecto eliminado ✨`,`success`)}catch{r(`Error al eliminar`)}})},w={instagram:`linear-gradient(45deg, #f58529, #dd2a7b, #8134af, #515bd4)`,tiktok:`#000`,youtube:`#ff0000`,whatsapp:`#25D366`,facebook:`#1877F2`,twitter:`#000`,linkedin:`#0077b5`,telegram:`#0088cc`,onlyfans:`#00aff0`,pinterest:`#E60023`,web:`#00a8e6`,link:`#FFDA34`},T=e=>e?e.includes(`instagram.com`)?`instagram`:e.includes(`tiktok.com`)?`tiktok`:e.includes(`youtube.com`)||e.includes(`youtu.be`)?`youtube`:/whatsapp|wa\.me/.test(e)?`whatsapp`:e.includes(`facebook.com`)||e.includes(`fb.me`)?`facebook`:/twitter\.com|x\.com/.test(e)?`twitter`:e.includes(`linkedin.com`)?`linkedin`:/t\.me|telegram/.test(e)?`telegram`:e.includes(`onlyfans.com`)?`onlyfans`:e.includes(`pinterest.com`)?`pinterest`:/\.(com|net|org|co|info|es|app|io|me)/.test(e)?`web`:`link`:`link`,E=()=>{let e=document.getElementById(`cr_phone_render`);if(!e)return;let t=g.find(e=>e.slug===_);if(!t){e.innerHTML=`<div class="ad_empty" style="margin-top:40%; font-size:0.75rem; color:var(--tx3);"><i class="fas fa-mobile-alt" style="font-size:2rem; display:block; margin-bottom:1vh; opacity:0.3;"></i>En vivo</div>`;return}let n=t.color||`#FFFFFF`,r=(t.links||[]).filter(e=>e.titulo).map(e=>{let t=e.icon||b(e.url),r=n;(!r||r.toLowerCase()===`#ffffff`)&&(r=w[T(e.url)]);let i=r&&r.includes(`gradient`);return`
  ...
  `};window.crCicloIcono=e=>{let t=g.find(e=>e.slug===_);if(!t)return;let n=t.links[e],r=n.icon||``,i=y.indexOf(r);i===-1&&(i=0);let a=y[(i+1)%y.length];n.icon=a;let o=document.getElementById(`icon_prev_${e}`);o&&(o.className=a||b(n.url)||`fas fa-link`),E()},window.crPreviewEnlace=(e,t)=>{let n=parseInt(e.dataset.idx),r=g.find(e=>e.slug===_);if(!r)return;let i=e.value;if(t===`url`&&i&&!i.startsWith(`http`)&&(i=`https://`+i),r.links[n][t]=i,t===`url`&&!r.links[n].icon){let e=document.getElementById(`icon_prev_${n}`);e&&(e.className=b(i)||`fas fa-link`)}E()},window.crPreviewApariencia=(e,t)=>{let n=g.find(e=>e.slug===_);n&&(n[t]=e.value,E())},window.crSeleccionar=e=>{_=e,S(),C(),E()},window.crCopiar=e=>{navigator.clipboard.writeText(`${window.location.origin}/${e}`).then(()=>r(`Enlace copiado 📋`,`success`))},window.crElegirColor=e=>{document.querySelectorAll(`.cr_swatch`).forEach(e=>e.classList.remove(`selected`)),e.classList.add(`selected`);let t=e.dataset.color;document.getElementById(`edit_color`).value=t;let n=g.find(e=>e.slug===_);n&&(n.color=t),E()};var D=()=>{let t=n(`wiSmile`);if(!t)return;let l=document.getElementById(`btn_nuevo_proy`),f=document.getElementById(`form_nuevo_proy`),v=document.getElementById(`inp_slug`);l?.addEventListener(`click`,()=>{f.style.display=`block`,v.focus()}),document.getElementById(`btn_cancel_proy`)?.addEventListener(`click`,()=>{f.style.display=`none`,v.value=``}),document.getElementById(`btn_save_proy`)?.addEventListener(`click`,async e=>{let n=v.value.trim().toLowerCase().replace(/[^a-z0-9-]/g,``);if(!n)return r(`Ingresa un slug válido`);let o=e.currentTarget,c=o.innerHTML;o.innerHTML=`<i class="fas fa-spinner fa-spin"></i>`,o.disabled=!0;try{let e=i(m,`linkwiis`,n);if((await u(e)).exists())return r(`Ese slug ya está ocupado. Elige otro.`);await a(e,{slug:n,usuario:t.usuario,email:t.email,estado:!0,vistas:0,desc:`¡Bienvenido a mi Linkwii!`,logo:t.avatar||``,color:`#FFFFFF`,links:[],creado:s(),actualizado:s()}),r(`Proyecto creado 🎉`,`success`),f.style.display=`none`,v.value=``,window.crSeleccionar(n)}catch(e){console.error(e),r(`Error al crear`)}finally{o.innerHTML=c,o.disabled=!1}});let y=c(p(m,`linkwiis`),o(`usuario`,`==`,t.usuario));g.length>0&&(S(),C(),E()),h=d(y,t=>{g=t.docs.map(e=>({slug:e.id,...e.data()})),e(`misProyectosLinkwii`,g),!_&&g.length>0?_=g[0].slug:_&&!g.find(e=>e.slug===_)&&(_=g.length>0?g[0].slug:null),S(),C(),E()})},O=()=>{h&&h(),delete window.crSeleccionar,delete window.crCopiar,delete window.crElegirColor};export{O as cleanup,D as init,x as render};