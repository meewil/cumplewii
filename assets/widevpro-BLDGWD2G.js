import{A as e,B as t,C as n,D as r,E as i,F as a,H as o,I as s,L as c,M as l,N as u,O as d,R as f,S as p,T as m,V as ee,W as h,_ as g,a as _,b as v,c as y,d as b,f as x,g as te,i as S,j as ne,k as C,l as re,m as w,n as T,o as E,p as D,r as O,s as k,t as A,u as j,v as M,w as N,x as P,y as F,z as I}from"./widev-Czq8ue36.js";import{n as L}from"./vendor-Dkcx8wGc.js";var R=()=>`
  ...
  `,z=()=>{let R=document.getElementById(`wvp_sandbox`),z=e=>{if(!R)return;R.innerHTML=``;let t=document.createDocumentFragment();for(let n=0;n<e;n++){let e=document.createElement(`div`);e.className=`wvp_dummy_item`,e.textContent=`Item ${n}`,t.appendChild(e)}R.appendChild(t)};document.getElementById(`btn_run_select`)?.addEventListener(`click`,()=>{z(1e3);let e=performance.now();document.querySelectorAll(`.wvp_dummy_item`).length;let t=performance.now()-e,n=performance.now();L(`.wvp_dummy_item`).length;let r=performance.now()-n,i=t.toFixed(4),a=r.toFixed(4);document.getElementById(`v_select_time`).textContent=`${i} ms`,document.getElementById(`j_select_time`).textContent=`${a} ms`;let o=Math.max(t,r,1e-4),s=t/o*100,c=r/o*100,l=document.getElementById(`v_select_bar`),u=document.getElementById(`j_select_bar`);l&&(l.style.width=`${s}%`),u&&(u.style.width=`${c}%`);let d=r/Math.max(t,1e-4),f=document.getElementById(`select_speedup`);f&&(f.innerHTML=`<i class="fas fa-bolt"></i> Vanilla JS es <strong>${d.toFixed(1)}x</strong> más rápido`)}),document.getElementById(`btn_run_style`)?.addEventListener(`click`,()=>{z(500);let e=document.querySelectorAll(`.wvp_dummy_item`),t=L(`.wvp_dummy_item`),n=performance.now();for(let t=0;t<10;t++)e.forEach(e=>e.classList.toggle(`wvp_active_test`));let r=performance.now()-n,i=performance.now();for(let e=0;e<10;e++)t.toggleClass(`wvp_active_test`);let a=performance.now()-i,o=r.toFixed(4),s=a.toFixed(4);document.getElementById(`v_style_time`).textContent=`${o} ms`,document.getElementById(`j_style_time`).textContent=`${s} ms`;let c=Math.max(r,a,1e-4),l=r/c*100,u=a/c*100,d=document.getElementById(`v_style_bar`),f=document.getElementById(`j_style_bar`);d&&(d.style.width=`${l}%`),f&&(f.style.width=`${u}%`);let p=a/Math.max(r,1e-4),m=document.getElementById(`style_speedup`);m&&(m.innerHTML=`<i class="fas fa-bolt"></i> Vanilla JS es <strong>${p.toFixed(1)}x</strong> más rápido`)}),document.getElementById(`btn_run_render`)?.addEventListener(`click`,()=>{if(!R)return;let e=performance.now();R.innerHTML=``;let t=``;for(let e=0;e<500;e++)t+=`<div class="wvp_dummy_card" style="padding: 10px; margin: 5px; border: 1px solid #ddd;">
      ...
      </div>`;R.innerHTML=t;let n=performance.now()-e,r=performance.now();R.innerHTML=``;for(let e=0;e<500;e++){let t=L(`<div>`).addClass(`wvp_dummy_card`).css({padding:`10px`,margin:`5px`,border:`1px solid #ddd`});t.append(L(`<strong>`).text(`Cumpleañero ${e}`)),t.append(L(`<span>`).text(`Faltan ${e%30} días`)),L(R).append(t)}let i=performance.now()-r,a=n.toFixed(4),o=i.toFixed(4);document.getElementById(`v_render_time`).textContent=`${a} ms`,document.getElementById(`j_render_time`).textContent=`${o} ms`;let s=Math.max(n,i,1e-4),c=n/s*100,l=i/s*100,u=document.getElementById(`v_render_bar`),d=document.getElementById(`j_render_bar`);u&&(u.style.width=`${c}%`),d&&(d.style.width=`${l}%`);let f=i/Math.max(n,1e-4),p=document.getElementById(`render_speedup`);p&&(p.innerHTML=`<i class="fas fa-bolt"></i> Vanilla JS es <strong>${f.toFixed(1)}x</strong> más rápido`),R.innerHTML=``}),document.getElementById(`btn_meta_run`)?.addEventListener(`click`,()=>{r({title:document.getElementById(`meta_title_inp`)?.value||`CumpleWii Pro`,desc:`Página de benchmark y utilidades mejorada a Vanilla JS`,keywords:`cumplewii, vanilla, benchmark, speed`,type:`Article`,datePublished:new Date});let e=document.getElementById(`log_meta`);e&&(e.innerHTML=`document.title = "${document.title}"<br>Schema Article inyectado en head.<br>Metas actualizados de forma reactiva.`),E(`Metadatos SEO actualizados`,`success`)});let B=null,V=2,H=document.getElementById(`wivista_scroll_container`),U=document.getElementById(`wivista_load_trigger`),W=()=>{!H||!U||(B=ee(`#wivista_load_trigger`,e=>{let t=document.getElementById(`log_wivista`);t&&(t.innerHTML+=`[wiVista] Detectado fin de página. Cargando Página ${V}...<br>`),setTimeout(()=>{let e=document.createElement(`div`);e.className=`scroll_page`,e.id=`page_${V}`,e.textContent=`📄 Página de Documento ${V} (Cargada inteligentemente con wiVista)`,H.insertBefore(e,U),t&&(t.innerHTML+=`[wiVista] Página ${V} renderizada suavemente.<br>`,t.scrollTop=t.scrollHeight),V++,V>4?(U.innerHTML=`<i class="fas fa-check-double"></i> Documento completo (4 páginas cargadas)`,B&&B.unobserve(U)):W()},700)},{root:H,once:!0,threshold:.05}))};W(),document.getElementById(`btn_herowi_run`)?.addEventListener(`click`,()=>{let e=document.getElementById(`herowi_container`);if(e){e.querySelectorAll(`.hwi_item`).forEach(e=>{e.classList.remove(`hwi`),delete e.dataset.hi}),n(`#herowi_container`,60);let t=document.getElementById(`log_herowi`);t&&(t.innerHTML=`Animación herowi aplicada.<br>Delays escalonados por index.`)}}),document.getElementById(`btn_showi_run`)?.addEventListener(`click`,()=>{let e=document.getElementById(`showi_container_box`);if(e){e.querySelectorAll(`.showi_item`).forEach(e=>{e.style.opacity=`0`,e.style.transform=`translateY(3vh)`,e.classList.add(`swi`),delete e.dataset.i}),d(`#showi_container_box`,120);let t=document.getElementById(`log_showi`);t&&(t.textContent=`[showi] Reiniciado. Elementos se deslizan en cascada y permanecen visibles.`)}}),n(`#herowi_container`,60),d(`#showi_container_box`,120),document.getElementById(`btn_spin_test`)?.addEventListener(`click`,e=>{let t=e.currentTarget;c(t,!0,`Guardando`);let n=document.getElementById(`log_spin`);n&&(n.textContent=`Estado: Cargando (disabled = true)...`),setTimeout(()=>{c(t,!1,`Guardar Cambios`),n&&(n.textContent=`Estado: Listo (restaurado).`)},1500)});let G=document.getElementById(`spy_scroll_viewport`);G&&(a([`spySec1`,`spySec2`,`spySec3`],`.spy_lnk`,{margin:`-10% 0px -50% 0px`,cls:`active`}),G.addEventListener(`scroll`,()=>{let e=document.querySelector(`.spy_lnk.active`),t=document.getElementById(`log_spy`);e&&t&&(t.textContent=`Sección activa: ${e.textContent} (${e.getAttribute(`href`)})`)})),document.getElementById(`btn_ls_save`)?.addEventListener(`click`,()=>{let e=document.getElementById(`ls_key`)?.value,t=document.getElementById(`ls_val`)?.value;if(e&&t){i(e,t,2);let n=document.getElementById(`log_ls`);n&&(n.textContent=`[savels] Guardado: ${e} -> ${t} (Expira en 2 horas)`)}}),document.getElementById(`btn_ls_get`)?.addEventListener(`click`,()=>{let e=document.getElementById(`ls_key`)?.value;if(e){let t=v(e),n=document.getElementById(`log_ls`);n&&(n.textContent=`[getls] Clave "${e}" = ${t===null?`null (expiró o no existe)`:`"${t}"`}`)}}),document.getElementById(`btn_ls_del`)?.addEventListener(`click`,()=>{let e=document.getElementById(`ls_key`)?.value;if(e){m(e);let t=document.getElementById(`log_ls`);t&&(t.textContent=`[removels] Removido: "${e}"`)}});let ie=()=>{let e=C.user,t=document.getElementById(`log_auth`);t&&(t.innerHTML=e?`Usuario: <strong>${e.nombre}</strong><br>Rol: ${e.rol}<br>Autenticado: Sí`:`Usuario: <strong>Ninguno (null)</strong><br>Autenticado: No`)},ae=C.on(e=>{ie()});document.getElementById(`btn_auth_login`)?.addEventListener(`click`,()=>{C.login({nombre:`Wilder Taype`,rol:`admin`,estado:`activo`}),E(`Sesión iniciada con wiAuth`,`success`)}),document.getElementById(`btn_auth_logout`)?.addEventListener(`click`,()=>{C.logout(),E(`Sesión cerrada con wiAuth`,`info`)}),document.getElementById(`btn_wismart_run`)?.addEventListener(`click`,()=>{let e=document.getElementById(`log_wismart`);s({fn:[async()=>{e&&(e.innerHTML+=`Smart Callback 1 ejecutado.<br>`)},async()=>{e&&(e.innerHTML+=`Smart Callback 2 ejecutado.<br>`)}]}),e&&(e.innerHTML=`wiSmart inicializado. Toca la pantalla, muévela o haz scroll para disparar callbacks.<br>`)}),document.getElementById(`btn_saludo_run`)?.addEventListener(`click`,()=>{let e=k(),t=C.user?.nombre||`Invitado`,n=document.getElementById(`log_saludo`);n&&(n.textContent=`${e}${t}!`)}),document.getElementById(`btn_notif_success`)?.addEventListener(`click`,()=>{E(`Operación procesada con éxito`,`success`);let e=document.getElementById(`log_notif`);e&&(e.textContent=`Notificación Success desplegada`)}),document.getElementById(`btn_notif_error`)?.addEventListener(`click`,()=>{E(`Algo salió mal en el servidor`,`error`);let e=document.getElementById(`log_notif`);e&&(e.textContent=`Notificación Error desplegada`)}),document.getElementById(`btn_notif_warning`)?.addEventListener(`click`,()=>{E(`Advertencia: espacio libre bajo`,`warning`);let e=document.getElementById(`log_notif`);e&&(e.textContent=`Notificación Warning desplegada`)}),document.getElementById(`btn_notif_info`)?.addEventListener(`click`,()=>{E(`Nueva actualización disponible`,`info`);let e=document.getElementById(`log_notif`);e&&(e.textContent=`Notificación Info desplegada`)}),document.getElementById(`btn_msg_success`)?.addEventListener(`click`,()=>{S(`Mensaje superior inyectado con Vanilla JS`,`success`);let e=document.getElementById(`log_msg`);e&&(e.textContent=`Mensaje Success activo.`)}),document.getElementById(`btn_msg_error`)?.addEventListener(`click`,()=>{S(`Error grave simulado por consola`,`error`);let e=document.getElementById(`log_msg`);e&&(e.textContent=`Mensaje Error activo.`)}),t(),document.getElementById(`btn_tip_manual`)?.addEventListener(`click`,e=>{t(e.currentTarget,`¡Tooltip manual por JavaScript!`,`success`,2e3);let n=document.getElementById(`log_tip`);n&&(n.textContent=`Tooltip programático lanzado en botón.`)}),document.getElementById(`btn_ip_fetch`)?.addEventListener(`click`,()=>{let e=document.getElementById(`log_ip`);e&&(e.textContent=`Consultando API de ipinfo...`),l(t=>{e&&(t?e.innerHTML=`<pre style="margin:0;font-size:11px;">IP: ${t.ip}
Ciudad: ${t.city}, ${t.country}
Dispositivo: ${t.device} (${t.os})
Browser: ${t.browser}
Z.H: ${t.timezone} (UTC ${t.utcOffset})
Online: ${t.online}</pre>`:e.textContent=`Error: No se pudieron obtener los detalles de red.`)})}),document.getElementById(`btn_modal_open`)?.addEventListener(`click`,()=>{y(`wvp_modal_demo`);let e=document.getElementById(`log_modal`);e&&(e.textContent=`Modal activo: wvp_modal_demo. CSS dinámico wiModal inyectado.`)}),document.getElementById(`btn_close_modal_test`)?.addEventListener(`click`,()=>{w(`wvp_modal_demo`);let e=document.getElementById(`log_modal`);e&&(e.textContent=`Modal cerrado mediante botón.`)}),document.getElementById(`btn_modal_x`)?.addEventListener(`click`,()=>{w(`wvp_modal_demo`);let e=document.getElementById(`log_modal`);e&&(e.textContent=`Modal cerrado mediante aspa superior.`)}),document.getElementById(`btn_date_parse`)?.addEventListener(`click`,()=>{let t=document.getElementById(`date_input_test`)?.value,n=document.getElementById(`log_date`);if(t&&n){let r=e({fromDate:e=>({seconds:Math.floor(e.getTime()/1e3)}),toDate:()=>new Date(t)}),i=r.save(t),a=r.get(i,`local`),o=r.get(i,`full`),s=r.get(i,`std`);n.innerHTML=`Entrada: ${t}<br>
Firebase seconds: ${i.seconds}<br>
get(local): ${a}<br>
get(full): ${o}<br>
get(std): ${s}`}}),document.getElementById(`btn_copy_run`)?.addEventListener(`click`,e=>{o(`#copy_text_inp`,e.currentTarget,`¡Texto Copiado!`);let t=document.getElementById(`log_copy`);t&&(t.textContent=`Texto enviado al portapapeles y notificado.`)});let K=0;f(`#btn_click_sum_target`,()=>{K=0;let e=document.getElementById(`btn_click_sum_target`);e&&(e.textContent=`Clicks Consecutivos (0/5)`);let t=document.getElementById(`log_wisuma`);t&&(t.textContent=`¡ÉXITO! Se alcanzaron 5 clics seguidos.`),E(`¡wiSuma detectó 5 clics rápidos!`,`success`)},5),document.getElementById(`btn_click_sum_target`)?.addEventListener(`click`,e=>{K++,e.currentTarget.textContent=`Clicks Consecutivos (${K}/5)`;let t=document.getElementById(`log_wisuma`);t&&(t.textContent=`Clic #${K} recibido.`)});let q=document.getElementById(`helpers_text_inp`),J=()=>{let e=q?.value||``,t=document.getElementById(`log_helpers`);t&&(t.innerHTML=`year(): ${h()}<br>
Mayu("${e}"): ${O(e)}<br>
Capi("${e}"): ${A(e)}<br>
mis10("${e}"): ${N(e)}`)};q?.addEventListener(`input`,J),J(),document.querySelectorAll(`.siblings_row .sib_box`).forEach(e=>{e.addEventListener(`click`,e=>{re(e.currentTarget,`active`);let t=document.getElementById(`log_adclass`);t&&(t.textContent=`[adrm] Activado el nodo #${e.currentTarget.id} y limpiados sus hermanos.`)})}),document.getElementById(`btn_adtm_test`)?.addEventListener(`click`,e=>{j(e.currentTarget,`flash_active`,`¡FLASH!`,`adtm (Flash)`);let t=document.getElementById(`log_adclass`);t&&(t.textContent=`[adtm] Clase flash_active inyectada por 1.8 segundos.`)}),document.getElementById(`btn_adup_test`)?.addEventListener(`click`,e=>{b(e.currentTarget,`¡ACTUALIZANDO!`),setTimeout(()=>{e.currentTarget.textContent=`adup (Actualizar)`},500);let t=document.getElementById(`log_adclass`);t&&(t.textContent=`[adup] Texto cambiado y clase class="updating" inyectada.`)}),document.getElementById(`btn_path_eval`)?.addEventListener(`click`,()=>{let e=document.getElementById(`log_path`);e&&(e.innerHTML=`wiPath.actual: "${u.actual}"<br>
wiPath.limpiar(location.pathname): "${u.limpiar(location.pathname)}"<br>
wiPath.params(): ${JSON.stringify(u.params())}`)});let Y=!1;document.getElementById(`btn_fade_run`)?.addEventListener(`click`,()=>{Y=!Y,ne(`#fade_sandbox_box`,Y?`<strong style="color:var(--mco);">Contenido Renovado Pro</strong>`:`Contenido Original`,120);let e=document.getElementById(`log_fade`);e&&(e.textContent=`Efecto de fade ejecutado en caja.`)});let X=document.getElementById(`name_helper_inp`),Z=()=>{let e=X?.value||``,t=document.getElementById(`log_names`);t&&(t.innerHTML=`Capit(): "${T(e)}"<br>
NombreApellido(): "${_(e)}"<br>
getNombre(): "${F(e)}"<br>
avatar(): "${x(e)}"`)};X?.addEventListener(`input`,Z),Z();let Q=document.getElementById(`rel_time_input`);if(Q){let e=new Date(Date.now()-3*3600*1e3);Q.value=new Date(e.getTime()-e.getTimezoneOffset()*6e4).toISOString().slice(0,16)}document.getElementById(`btn_rel_time_run`)?.addEventListener(`click`,()=>{let e=Q?.value,t=document.getElementById(`log_dates_comp`);if(e&&t){let n=new Date(e).getTime(),r=I({seconds:Math.floor(n/1e3)}),i=g({seconds:Math.floor(n/1e3)}),a=D(n);t.innerHTML=`fechaHoy(): "${te()}"<br>
formatearFechaParaInput(): "${M(n)}"<br>
formatearFechaHora(): "${i}"<br>
wiTiempo(): "${r}"<br>
calcMeses(): ${a} meses de diferencia`}});let $=document.getElementById(`autosave_inp`);$&&(P(`test_autosave`,e=>{$.value=e;let t=document.getElementById(`log_autosave`);t&&(t.textContent=`[getsave] Recuperado: "${e}"`)}),p(`test_autosave`,$.value),$.addEventListener(`input`,e=>{i(`test_autosave`,e.target.value,168);let t=document.getElementById(`log_autosave`);t&&(t.textContent=`[savels] Guardado en vivo: "${e.target.value}"`)})),document.getElementById(`btn_autosave_sim`)?.addEventListener(`click`,()=>{let e=v(`test_autosave`),t=document.getElementById(`log_autosave`);t&&(t.textContent=e?`[getls] Resguardo activo: "${e}"`:`[getls] Resguardo vacío. Escribe algo en el input para resguardar.`)}),window._wvp_unbind_auth=ae,window._wvp_obs_wivista=B,console.log(`⚡ Módulo widevpro cargado en Vanilla JS con 100% de la suite.`)},B=()=>{let e=document.getElementById(`wvp_sandbox`);e&&(e.innerHTML=``),window._wvp_unbind_auth&&(window._wvp_unbind_auth(),delete window._wvp_unbind_auth),window._wvp_obs_wivista&&(window._wvp_obs_wivista.disconnect?.(),delete window._wvp_obs_wivista)};export{B as cleanup,z as init,R as render};