import{l as e}from"./wii-DFanSMW3.js";import{B as t,L as n,N as r,b as i,i as a,k as o,o as s}from"./widev-Czq8ue36.js";import{n as c}from"./vendor-Dkcx8wGc.js";import{r as l}from"./index-U_klNYw1.js";import{A as u,C as d,N as f,T as p,_ as m}from"./firebase-BY5GISzq.js";import{n as h}from"./firebase-BOoMX3kr.js";import{a as g,b as _,o as v,t as y,u as b}from"./devblog-CBj5FplG.js";var x=e=>e.trim().toLowerCase().normalize(`NFD`).replace(/[\u0300-\u036f]/g,``).replace(/\b(el|la|los|las|de|del|en|un|una|y|a|con|por|para|que|es|se)\b/g,` `).replace(/[^a-z0-9\s]/g,``).replace(/\s+/g,`_`).replace(/_{2,}/g,`_`).replace(/^_|_$/g,``).slice(0,50),S=e=>{let t=e.replace(/<[^>]*>/g,` `).split(/\s+/).filter(Boolean);return{words:t.length,min:Math.max(1,Math.ceil(t.length/200))}},C=()=>r.params()?.edit||new URLSearchParams(location.search).get(`edit`)||null,w=()=>{let n=o.user?.usuario?o.user:i(`wiSmile`)||{};if(!n.email)return`<div class="nu_err dpvc"><i class="fas fa-lock"></i><h2>Acceso restringido</h2><p>Inicia sesión para crear historias</p></div>`;let r=C();return`
  <div class="nu_wrap">
    <div class="nu_head">
...
`)},k=w?`wi_draft_edit_`+e:`wi_draft_new`,A=()=>{let e={titulo:c(`#nu_titulo`).val(),slug:c(`#nu_slug_inp`).val(),resumen:c(`#nu_resumen`).val(),keywords:c(`#nu_keywords`).val(),cat:c(`#nu_cat_inp`).val(),img:c(`#nu_img`).val(),imgTop:c(`#nu_img_top`).val(),content:c(`#nu_contenido`).val(),tags:t};localStorage.setItem(k,JSON.stringify(e))};if(c(`#nu_form`).on(`input`,`input, textarea`,()=>{clearTimeout(window.wiDraftTimer),window.wiDraftTimer=setTimeout(A,1e3)}),(()=>{try{let e=new Set,t=new Set;for(let n of Object.keys(localStorage).filter(e=>e.startsWith(`wi_blogs`)||e.startsWith(`wi_post_`))){let r=i(n);(Array.isArray(r)?r:r?[r]:[]).forEach(n=>{n.categoria&&e.add(n.categoria),n.tags&&Array.isArray(n.tags)&&n.tags.forEach(e=>t.add(e))})}e.size>0&&c(`#nu_cat_sug`).html(Array.from(e).slice(0,8).map(e=>`<span class="nu_sug_chip cat_sug">${e}</span>`).join(``)),t.size>0&&c(`#nu_tag_sug`).html(Array.from(t).slice(0,12).map(e=>`<span class="nu_sug_chip tag_sug">#${e}</span>`).join(``))}catch{console.warn(`No se pudieron cargar sugerencias`)}})(),w)try{let n=await b(e,!0);if(!n?.data){s(`Post no encontrado`,`error`);return}let r=n.data;c(`#nu_titulo`).val(r.titulo),c(`#nu_slug_inp`).val(r.slug||r.id),c(`#nu_resumen`).val(r.resumen||``).trigger(`input`),c(`#nu_keywords`).val(r.keywords||``),c(`#nu_contenido`).val(r.contenidoMd||r.contenido||``),c(`#nu_img`).val(r.imagen||``),c(`#nu_img_top`).val(r.imagenTop||``),c(`#nu_activo`).prop(`checked`,r.activo!==!1),c(`#nu_pin`).prop(`checked`,!!r.pin),c(`#nu_cat_inp`).val(r.categoria||``),t=Array.isArray(r.tags)?[...r.tags]:[],D(),r.imagen&&(c(`#nu_img_el`).attr(`src`,r.imagen),c(`#nu_img_prev`).removeClass(`dpn`)),r.imagenTop&&(c(`#nu_img_top_el`).attr(`src`,r.imagenTop),c(`#nu_img_top_prev`).removeClass(`dpn`)),c(`#nu_resumen_cnt`).text((r.resumen||``).length),E()}catch(e){console.error(`edit load:`,e),s(`Error cargando post`,`error`)}else try{let e=JSON.parse(localStorage.getItem(k));e&&(c(`#nu_titulo`).val(e.titulo),c(`#nu_slug_inp`).val(e.slug),c(`#nu_resumen`).val(e.resumen),c(`#nu_keywords`).val(e.keywords||``),c(`#nu_cat_inp`).val(e.cat),c(`#nu_img`).val(e.img).trigger(`input`),c(`#nu_img_top`).val(e.imgTop).trigger(`input`),c(`#nu_contenido`).val(e.content),e.tags&&(t=e.tags,D()),E())}catch{}c(`#nu_form`).on(`keydown`,e=>{e.key===`Enter`&&e.target.tagName!==`TEXTAREA`&&e.preventDefault()}),w||(c(`#nu_slug_inp`).on(`input`,function(){c(this).val(c(this).val().replace(/[^a-z0-9_]/gi,e=>e===` `?`_`:``).toLowerCase().replace(/_{2,}/g,`_`)),c(`#nu_slug_status`).html(`<i class="fas fa-pen"></i> Escribiendo...`).removeClass(`ok err`)}).on(`change`,async function(){let e=c(this).val(),t=c(`#nu_slug_status`);if(!e)return t.html(``).removeClass(`ok err`);if(e.length<3)return t.html(`<i class="fas fa-exclamation"></i> Muy corto`).addClass(`err`).removeClass(`ok`);t.html(`<i class="fas fa-spinner fa-spin"></i>`).removeClass(`ok err`),(await m(u(h,`blog`,e)).catch(()=>null))?.exists()?t.html(`<i class="fas fa-xmark"></i> Ya existe`).addClass(`err`).removeClass(`ok`):t.html(`<i class="fas fa-check"></i> OK`).addClass(`ok`).removeClass(`err`)}),c(`#nu_slug_reset`).on(`click`,()=>{c(`#nu_slug_inp`).val(T()).trigger(`input`).trigger(`change`)})),c(`#nu_resumen`).on(`input`,function(){c(`#nu_resumen_cnt`).text(c(this).val().length)}),c(`#nu_img`).on(`input`,function(){clearTimeout(r),r=setTimeout(()=>{let e=c(this).val().trim();if(!e)return c(`#nu_img_prev`).addClass(`dpn`);c(`#nu_img_el`).attr(`src`,e).off(`load error`).on(`load`,()=>c(`#nu_img_prev`).removeClass(`dpn`)).on(`error`,()=>c(`#nu_img_prev`).addClass(`dpn`))},600)}),c(`#nu_img_clear`).on(`click`,()=>{c(`#nu_img`).val(``).trigger(`input`),c(`#nu_img_prev`).addClass(`dpn`)});let j;c(`#nu_img_top`).on(`input`,function(){clearTimeout(j),j=setTimeout(()=>{let e=c(this).val().trim();if(!e)return c(`#nu_img_top_prev`).addClass(`dpn`);c(`#nu_img_top_el`).attr(`src`,e).off(`load error`).on(`load`,()=>c(`#nu_img_top_prev`).removeClass(`dpn`)).on(`error`,()=>c(`#nu_img_top_prev`).addClass(`dpn`))},600)}),c(`#nu_img_top_clear`).on(`click`,()=>{c(`#nu_img_top`).val(``).trigger(`input`),c(`#nu_img_top_prev`).addClass(`dpn`)}),c(`#nu_contenido`).on(`input`,E),c(`#nu_tags_inp`).on(`keydown`,function(e){e.key!==`Enter`&&e.key!==`,`||(e.preventDefault(),c(this).val().toLowerCase().split(`,`).map(e=>e.trim().replace(/\s+/g,`_`)).filter(Boolean).forEach(e=>{!t.includes(e)&&t.length<8&&t.push(e)}),D(),c(this).val(``))}),c(document).on(`click.nuevo`,`.nu_tool`,function(){let e=c(this).data(`tag`),t=c(`#nu_contenido`),n=t[0],r=n.selectionStart,i=n.selectionEnd,a=n.value.substring(r,i)||`texto`,o=e.replace(`texto`,a).replace(`cita`,a);t.val(n.value.substring(0,r)+o+n.value.substring(i)),n.focus(),n.selectionStart=r,n.selectionEnd=r+o.length,E(),A()}).on(`click.nuevo`,`.nu_tab`,function(){let e=c(this).data(`tab`);c(`.nu_tab`).removeClass(`active`),c(this).addClass(`active`),e===`prev`?(c(`#nu_prev_html`).html(_(O(c(`#nu_contenido`).val()))).removeClass(`dpn`),c(`#nu_contenido`).addClass(`dpn`)):(c(`#nu_contenido`).removeClass(`dpn`),c(`#nu_prev_html`).addClass(`dpn`))}).on(`click.nuevo`,`.nu_tag_rm`,function(){t.splice(+c(this).data(`i`),1),D()}).on(`click.nuevo`,`.cat_sug`,function(){c(`#nu_cat_inp`).val(c(this).text())}).on(`click.nuevo`,`.tag_sug`,function(){let e=c(this).text().replace(`#`,``);e&&!t.includes(e)&&t.length<8&&(t.push(e),D())}),c(`#nu_form`).on(`submit`,async function(r){r.preventDefault();let b=c(`#nu_submit,.nu_btn_full`),x=o.user?.usuario?o.user:i(`wiSmile`)||{},C=c(`#nu_cat_inp`).val().trim();C&&=C.charAt(0).toUpperCase()+C.slice(1).toLowerCase();let[T,E,D,A,j,M,N,P]=[c(`#nu_titulo`).val().trim(),c(`#nu_resumen`).val().trim(),c(`#nu_keywords`).val().trim(),C,c(`#nu_img`).val().trim(),c(`#nu_img_top`).val().trim(),c(`#nu_contenido`).val().trim(),c(`#nu_slug_inp`).val().trim()],F=_(O(N));if(!T||!E||!A||!j||!N)return s(`Completa los campos obligatorios`,`warning`);if(N.length<10)return s(`Contenido muy corto`,`warning`);if(!P||P.length<3)return s(`Slug inválido`,`warning`);if(!w&&c(`#nu_slug_status`).hasClass(`err`))return s(`Slug no disponible`,`error`);n(b,!0,w?`Guardando...`:`Publicando...`);try{if(`${S(F).min}`,w)await p(u(h,y,e),{activo:c(`#nu_activo`).is(`:checked`),pin:c(`#nu_pin`).is(`:checked`),titulo:T,resumen:E,keywords:D,categoria:A,contenido:F,contenidoMd:N,imagen:j,imagenTop:M,imagenAlt:T,tags:t,tiempoLectura:`${S(F).min} min`,actualizado:f()}),v(e),g(),a(`¡Historia actualizada! 🐾✨`,`success`),setTimeout(()=>l(()=>import(`./index-U_klNYw1.js`).then(t=>t.rutas.navigate(`/${e}`)),[]),1200);else{if((await m(u(h,`blog`,P))).exists())return n(b,!1),s(`Slug existente`,`warning`);await d(u(h,y,P),{id:P,slug:P,activo:c(`#nu_activo`).is(`:checked`),pin:c(`#nu_pin`).is(`:checked`),usuario:x.usuario,email:x.email,autor:x.nombre||x.usuario,titulo:T,resumen:E,keywords:D,categoria:A,contenido:F,contenidoMd:N,imagen:j,imagenTop:M,imagenAlt:T,tags:t,vistas:0,likes:0,tiempoLectura:`${S(F).min} min`,creado:f(),actualizado:f()}),g(),localStorage.removeItem(k),a(`¡Historia publicada! 🐾✨`,`success`),setTimeout(()=>l(()=>import(`./index-U_klNYw1.js`).then(e=>e.rutas.navigate(`/${P}`)),[]),1200)}}catch(e){console.error(`nuevo:`,e),s(w?`Error al guardar`:`Error al publicar`,`error`),n(b,!1)}})},E=()=>{c(`#nu_form,#nu_slug_inp,#nu_titulo,#nu_resumen,#nu_img,#nu_contenido`).off(),c(document).off(`.nuevo`)};export{E as cleanup,T as init,w as render};