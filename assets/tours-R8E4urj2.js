import{k as e}from"./widev-Czq8ue36.js";import{n as t}from"./vendor-Dkcx8wGc.js";import{n}from"./index-U_klNYw1.js";import{cargarTours as r}from"./zsmile-CA_7WcPo.js";var i=[],a=()=>`
...
`,o=async()=>{if(!e.user)return setTimeout(()=>n.navigate(`/login`),100);c(),t(document).off(`click.tours_events`,`.smw_tab_btn`).on(`click.tours_events`,`.smw_tab_btn`,function(){let e=t(this).data(`tab`);t(`.smw_tab_btn`).removeClass(`active`),t(this).addClass(`active`),t(`.smw_tab_pane`).removeClass(`active`),t(`#pane-${e}`).addClass(`active`)}),t(`.wi_fadeUp`).addClass(`visible wi_visible`),window.__WIREADY__=!0},s=()=>{t(document).off(`.tours_events`)};async function c(){let e=t(`.smw_tours_header`);e.addClass(`smw_loading`);try{if(t(`#preciosCatalogGrid`).html(l(4)),t(`#puntosCatalogGrid`).html(l(4)),i=await r(),!i.length){t(`#preciosCatalogGrid`).html(`<div class="smw_empty_pane">No hay tours cargados en la base de datos.</div>`),t(`#puntosCatalogGrid`).html(`<div class="smw_empty_pane">No hay tours cargados en la base de datos.</div>`);return}let e=i.map(e=>`
    ...
    `).join(``);t(`#preciosCatalogGrid`).html(e);let n=i.map(e=>`
    ...
    `).join(``);t(`#puntosCatalogGrid`).html(n)}catch(e){console.error(`Error al cargar catalogo:`,e),t(`#preciosCatalogGrid`).html(`<div class="smw_empty_pane">Error al cargar información de tours.</div>`)}finally{e.removeClass(`smw_loading`)}}function l(e=4){return Array(e).fill(0).map(()=>`
