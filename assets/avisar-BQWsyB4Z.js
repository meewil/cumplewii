import{b as e,o as t,t as n}from"./widev-Czq8ue36.js";import{n as r}from"./vendor-Dkcx8wGc.js";import{n as i}from"./index-U_klNYw1.js";import{A as a,C as o,N as s,S as c,k as l,v as u,x as d}from"./firebase-BY5GISzq.js";import{n as f,t as p}from"./firebase-BOoMX3kr.js";var m=`bug`,h=[],g=()=>{let t=e(`wiSmile`)||{};return t.nombre||t.usuario||p.currentUser?.displayName||p.currentUser?.email||`Anónimo`},_=()=>`
  ...
  `,v=async()=>{if(!p.currentUser&&!e(`wiSmile`))return setTimeout(()=>i.navigate(`/login`),100);r(`.wi_fadeUp`).addClass(`visible wi_visible`),await x(),b(),console.log(`🐛 SPA Feedback & Issues cargado exitosamente`),window.__WIREADY__=!0},y=()=>{r(document).off(`.avisar_events`),window.toggleExpandirIssue=null};function b(){r(document).off(`click.avisar_events`,`.smw_cat_tag_btn`).on(`click.avisar_events`,`.smw_cat_tag_btn`,function(){r(`.smw_cat_tag_btn`).removeClass(`active`),r(this).addClass(`active`),m=r(this).data(`cat`)}),r(document).off(`submit.avisar_events`,`#frmFeedback`).on(`submit.avisar_events`,`#frmFeedback`,async function(e){e.preventDefault();let n=r(`#fbTitle`).val().trim(),i=r(`#fbDesc`).val().trim();if(!n||!i)return t(`Completa todos los campos obligatorios.`,`warning`);await C(n,i)}),r(document).off(`click.avisar_events`,`#btnSyncIssues`).on(`click.avisar_events`,`#btnSyncIssues`,async function(){r(this).find(`i`).addClass(`fa-spin`),await x(),r(this).find(`i`).removeClass(`fa-spin`),t(`Feed sincronizado exitosamente`,`success`)}),window.toggleExpandirIssue=e=>{r(`.smw_issue_card[data-id="${e}"]`).toggleClass(`expanded`)}}async function x(){let e=r(`.smw_avisar_header`);e.addClass(`smw_loading`);try{r(`#issuesList`).html(O(3)),h=(await u(c(l(f,`wiFeedbackIssues`),d(`fecha`,`desc`)))).docs.map(e=>({id:e.id,...e.data()})),S()}catch(e){console.error(`Error al cargar reportes:`,e),r(`#issuesList`).html(`
      <div class="smw_empty_pane">
        <i class="fas fa-exclamation-triangle"></i>
        <span>Error al cargar reportes. Revisa tu conexión.</span>
      </div>
    `)}finally{e.removeClass(`smw_loading`)}}function S(){if(r(`#lblIssuesCount`).text(`${h.length} reportes`),!h.length){r(`#issuesList`).html(`
      <div class="smw_empty_pane">
        <i class="fas fa-folder-open"></i>
        <strong>No hay reportes de feedback</strong>
        <p>Sé el primero en reportar un bug o sugerir una mejora.</p>
      </div>
    `);return}let e=h.map(e=>{let t=n(e.autor||`Colaborador`),r=w(e.fecha),i=T(e.estado),a=E(e.categoria);return`
