...
`,o=null,s=()=>{o=new IntersectionObserver(t=>t.forEach(t=>{t.isIntersecting&&e(t.target).addClass(`visible`)}),{threshold:.1}),e(`.wi_fadeUp`).each(function(){o.observe(this)}),e(document).on(`click.privacidad`,`.tm_nav`,function(t){t.preventDefault(),r(()=>import(`./index-kLqLg-A_.js`).then(t=>t.rutas.navigate(e(this).attr(`href`))),[])}),e(document).on(`click.privacidad`,`.tm_index_item`,function(t){t.preventDefault();let n=document.querySelector(e(this).attr(`href`));n&&window.scrollTo({top:n.getBoundingClientRect().top+scrollY-90,behavior:`smooth`})}),console.log(`🔒 ${t} Privacidad cargada`),window.__WIREADY__=!0},c=()=>{o?.disconnect?.(),o=null,e(document).off(`.privacidad`)};export{c as cleanup,s as init,a as render};